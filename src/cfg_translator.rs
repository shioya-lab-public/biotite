...
impl CfgTranslator {
    ...
    pub fn run(&mut self, cfg: Cfg) -> Program {
        cfg.into_iter()
            .map(|f| self.translate_function(f))
            .collect()
    }

    fn translate_function(&mut self, function: RiscvFunction) -> LlvmFunction {
        let RiscvFunction {
            name,
            basic_blocks,
            potential_targets,
        ...
        let mut body: Vec<_> = basic_blocks
            .into_iter()
            .enumerate()
            .map(|(i, block)| self.translate_basic_block(i, block, &potential_targets))
            .flatten()
            .collect();
